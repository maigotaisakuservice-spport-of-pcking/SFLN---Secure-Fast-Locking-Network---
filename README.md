...
## 🎨 必要な画像アセット (Required Assets)
ビルドおよびドキュメントのため、以下の画像を `resources/` ディレクトリに用意してください。

1.  **sfln_logo.png** (1024x1024, 背景透過): プロジェクトのメインロゴ。
2.  **sfln.ico** (256x256): Windows クライアントの実行ファイル用アイコン。
3.  **sfln_tray.png** (32x32, 単色推奨): システムトレイ（タスクトレイ）用アイコン。
4.  **android_icon.png** (512x512): Android アプリのランチャーアイコン。
5.  **screenshots/**: `transfer_demo.png`, `vpn_active.png` などの機能紹介用。
...
- **43-Byte Binary Header**: High-efficiency packet structure.
...
For builds and documentation, please prepare the following images in the `resources/` directory:

1.  **sfln_logo.png** (1024x1024, Transparent): The main project logo.
2.  **sfln.ico** (256x256): Executable icon for the Windows client.
3.  **sfln_tray.png** (32x32, Monochrome recommended): System tray icon.
4.  **android_icon.png** (512x512): Launcher icon for the Android app.
5.  **screenshots/**: `transfer_demo.png`, `vpn_active.png`, etc., for feature showcase.
