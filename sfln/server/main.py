        ...
        try:
            async for message in websocket:
                if isinstance(message, str):
                    ...
                    # SFLN-P v2 Format: [Magic(1)][Ver/Type(1)][Hop(1)][SrcID(16)][TargetID(16)][Seq(4)][Total(4)][Payload...]
                    if len(message) > 43 and message[0] == 0x53:
                        target_id_bytes = message[19:35]
    ...
    async def relay_data(self, target_id, packet):
        ...
        if target_id in self.udp_peers:
            try:
                ...
                self.logger.error(f"UDP Relay send failed: {e}")

        # Relay to WS
        if target_id in self.ws_peers:
            ws, nid_str, _ = self.ws_peers[target_id]
            try:
                # Use wait_for to prevent a single slow peer from blocking the entire relay process
                await asyncio.wait_for(ws.send(packet), timeout=2.0)
            except asyncio.TimeoutError:
                self.logger.warning(f"WS Relay timeout for node {nid_str}")
            except Exception as e:
                self.logger.error(f"WS Relay send failed for node {nid_str}: {e}")
...
class SFLNServerProtocol(asyncio.DatagramProtocol):
    ...
    def datagram_received(self, data, addr):
        ...
        if len(data) > 43 and data[0] == 0x53:
            target_id = data[19:35]
