/**
 * SFLN Node.js / Browser Library
 * v2.2.1 - Self-contained
 */
...
class SFLNCryptoJS {
    constructor(masterKey = null) {
        ...
        // Node.js vs Browser crypto handling
        this.cryptoApi = (typeof window !== 'undefined' && window.crypto) ? window.crypto : null;
        if (!this.cryptoApi && typeof require !== 'undefined') {
            try { this.cryptoApi = require('crypto').webcrypto; } catch(e) {}
        }
        if (!this.cryptoApi && typeof crypto !== 'undefined') this.cryptoApi = crypto;

        if (masterKey) this.masterKey = masterKey;
        else this.masterKey = this.cryptoApi.getRandomValues(new Uint8Array(this.KEY_BITS / 8));
    }
    ...
    async deriveChunkKey(chunkIndex, chunkSeed) {
        ...
        const hashBuffer = await this.cryptoApi.subtle.digest('SHA-256', combined);
        return await this.cryptoApi.subtle.importKey('raw', hashBuffer, { name: 'AES-GCM' }, false, ['encrypt', 'decrypt']);
    }
    ...
    async encryptData(data, progressCallback = null) {
        ...
        for (let i = 0; i < data.length; i += this.CHUNK_SIZE) {
            ...
            const chunkSeed = this.cryptoApi.getRandomValues(new Uint8Array(this.KEY_EMBED_SIZE));
            const key = await this.deriveChunkKey(idx, chunkSeed);
            const nonce = this.cryptoApi.getRandomValues(new Uint8Array(12));
            const encryptedBuffer = await this.cryptoApi.subtle.encrypt({ name: 'AES-GCM', iv: nonce }, key, chunk);
    ...
    async decryptChunk(chunkData, index) {
        ...
        const decryptedBuffer = await this.cryptoApi.subtle.decrypt({ name: 'AES-GCM', iv: nonce }, key, encryptedPayload);
...
class SFLNClientJS {
    constructor(masterKey = null, nodeId = null) {
        ...
        this.WSProvider = (typeof window !== 'undefined' && window.WebSocket) ? window.WebSocket : null;
        if (!this.WSProvider && typeof require !== 'undefined') {
            try { this.WSProvider = require('ws'); } catch(e) {}
        }
    }
    ...
    async connect(serverUrl) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!this.WSProvider) return reject(new Error("WebSocket provider not available"));

            this.ws = new this.WSProvider(wsUrl);
