...
class SFLNProtocol:
    ...
    def _create_header(self, ptype, target_id_bytes, seq, total, hop_count=0):
        ver_type = (0x2 << 4) | (ptype & 0xF)
        return struct.pack("!BBB16s16sII", self.MAGIC, ver_type, hop_count, self.node_id_bytes, target_id_bytes, seq, total)

    async def send_data(self, encrypted_chunks, target_peer_id, target_address, progress_callback=None, hop_count=0):
        ...
        async def worker(queue):
            ...
            while not queue.empty():
                ...
                try:
                    header = self._create_header(0, target_id_bytes, idx, total_chunks, hop_count=hop_count)
                    packet = header + chunk
    ...
    async def receive_data(self, expected_chunks_count, progress_callback=None):
        ...
        while len(received_chunks) < expected_chunks_count:
            ...
            if len(data) < 43 or data[0] != self.MAGIC: continue
            _, ver_type, hop, src_id, tgt_id, seq, total = struct.unpack("!BBB16s16sII", data[:43])
            received_chunks[seq] = data[43:]
